...
@section('page_header', 'Edit Laboratory Specifications')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="glass-panel rounded-2xl p-8 border border-slate-800">
        <h2 class="text-xl font-extrabold text-white mb-6">Modify Laboratory Specifications</h2>
        ...
        <form action="{{ route('laboratories.update', $laboratory->id) }}" method="POST" class="space-y-6" 
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" required
                    class="mt-2 block w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">{{ old('description', $laboratory->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="github_repo_template" id="github_repo_template" placeholder="owner/repo" value="{{ old('github_repo_template', $laboratory->github_repo_template) }}" 
                        class="mt-2 block w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
