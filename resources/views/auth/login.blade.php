...
<head>
    ...
    <title>Login - Certicode Labs</title>
    <!-- Tailwind CSS CDN -->
...
<body class="h-full flex overflow-hidden">
        ...
        <div class="max-w-xl space-y-6 z-10">
            ...
            <ul class="space-y-4 pt-4">
                <li class="flex items-start space-x-3 text-xs">
                    ...
                    <div>
                        <span class="font-bold text-white block">NetAcad Module Framework</span>
                        <span class="text-slate-400">Classes organize theoretical lessons and interactive laboratories sequentially.</span>
                    </div>
                </li>
                <li class="flex items-start space-x-3 text-xs">
                    ...
                    <div>
                        <span class="font-bold text-white block">AI Telemetry Integrity Detector</span>
                        <span class="text-slate-400">Integrated presence webcam monitor and tab tracking guarantees workspace compliance.</span>
                    </div>
                </li>
            </ul>
        </div>
    ...
    <div class="w-full md:w-1/2 lg:w-2/5 flex flex-col justify-center bg-[#0D1117] px-8 sm:px-16 md:px-12 lg:px-16 xl:px-20 overflow-y-auto">
        <div class="max-w-md w-full mx-auto space-y-6 py-12">
            ...
            <!-- Login Form -->
            <form class="space-y-4 pt-2" method="POST" action="{{ route('login.store') }}">
                @csrf
            ...
            <div>
                <a href="{{ route('auth.google') }}" class="w-full flex items-center justify-center py-2.5 px-4 border border-slate-800 rounded-lg text-sm font-semibold text-white bg-slate-900 hover:bg-slate-850 transition-colors shadow-md">
                    <!-- Google logo SVG -->
                    <svg class="h-4 w-4 mr-2.5" viewBox="0 0 24 24" width="24" height="24">
